...
public class TExecutive_Perception_Function extends TAgent_Base_Thread
{
	private TAgent agent;
	private TPerceptions_Processing Perceptions_Processing = null;
	private TInformation_Extraction Information_Extraction = null;
	private TStimulus_Filtering Stimulus_Filtering = null;
	...
	public TExecutive_Perception_Function( TAgent agent)
	{
		super(agent, "Executive Perception Function");
		this.Agent = agent;
		...
		this.Perceptions_Processing = new TPerceptions_Processing( this );
		this.Information_Extraction = new TInformation_Extraction( this );
		this.Stimulus_Filtering = new TStimulus_Filtering( this );
...
//		while( this.Read_Continue_Thread() )
		{
			...
			if (this.Message_Handler.Read_Value_And_Clear_Updated_Unhinibited_Data() )
			{
				...
				this.Inhibited_Beliefs.addAll(this.Global_Workspace.Get_Inhibited_Beliefs_from_MM());
//				this.Inhibited_Beliefs.addAll( this.Long_Term_Memory_Maintenance.Get_Inhibited_Beliefs());
				
				this.Inhibited_Regions.clear();
				this.Inhibited_Regions.addAll( this.Global_Workspace.Get_Inhibited_Regions_from_MM());
//				this.Inhibited_Regions.addAll( this.Long_Term_Memory_Maintenance.Get_Inhibited_Regions());
				
				this.Uninhibited_Beliefs.clear();
//				this.Uninhibited_Beliefs.addAll( this.Long_Term_Memory_Maintenance.Get_All_Beliefs() );
				this.Uninhibited_Beliefs.addAll( this.Global_Workspace.Get_All_Beliefs_from_MM() );
				this.Uninhibited_Beliefs.removeAll( this.Inhibited_Beliefs );
				
				this.Uninhibited_Regions.clear();
//				this.Uninhibited_Regions.addAll( this.Long_Term_Memory_Maintenance.Get_Regions() );
				this.Uninhibited_Regions.addAll( this.Global_Workspace.Get_All_Regions_from_MM());
			...
			this.Filtered_Stimuli.clear();
				...
				if (this.Stimuli.size() >0 )
				{
					...
					this.Filtered_Stimuli.addAll( this.Stimulus_Filtering.Get_Survived_Stimuli() );
				}
...
//			}
			if( this.Filtered_Stimuli.size() > 0)
				{
					this.Global_Workspace.Set_Filtered_Stimuli( this.Filtered_Stimuli );
				}
		}
	}
	...
	{
		this.Agent.Get_Global_WorkSpace().Insert_for_Update_Contract(TType_Update_Contract.Updated_Perceptions, this);
