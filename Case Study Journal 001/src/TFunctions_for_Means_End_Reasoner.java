...
//	}
	...
	public ArrayList<TOption> Reasoner_Function_for_Refuel_new
...

	{
		...
		//I get Beliefs
		TBelief BL_Current_Position_City = Beliefs.get("BL_Position_City");
		TBelief BL_Map = Beliefs.get("BL_Map");
		TBelief BL_Current_Time = Beliefs.get("BL_Current_Time");
		TBelief BL_Current_Position_Route = Beliefs.get("BL_Position_Route");
		TBelief BL_Current_Position_Step = Beliefs.get("BL_Position_Step");
		TBelief BL_Next_Position_Route = Beliefs.get("BL_Next_Position_Route");
		...
		Integer Current_City_Position = (Integer) BL_Current_Position_City.Get_Predicate()
				.Get_Object_Complement();
		TCity Current_City = TCity.values()[Current_City_Position];
		TCity Current_City_Name = TCity.values()[Current_City_Position];
		TCity Destination_City = TCity.values()[Current_City_Position];
		
		
		
		Integer Current_Route_Number = (Integer) BL_Current_Position_Route.Get_Predicate()
				.Get_Object_Complement();
		
		if(Current_Route_Number == -1)
		{
			Current_Route_Number = (Integer) BL_Next_Position_Route.Get_Predicate()
					.Get_Object_Complement();	
		}
		
		Integer Current_Step_Number = (Integer) BL_Current_Position_Step.Get_Predicate()
				.Get_Object_Complement();
		TRoute A_Route = null;;
		if(Current_Route_Number <= Map.All_Routes.size())
		{
			A_Route = Map.All_Routes.get(Current_Route_Number);
		}
		else
		{
			//Here is an error
		...
		Integer Specular_Route_Number = Map.Get_Specular_Route( Current_Route_Number );
		Integer Specular_Step_Number = Map.Get_Specular_Step_in_Route( 
												Current_Route_Number, Current_Step_Number );
		
		int Steps_on_route = A_Route.Get_Steps_Number();
		int Route_Speed = A_Route.Get_Route_Speed();
		double middle_on_the_route = (double)((double)Steps_on_route/2);
		Boolean Make_Inversion = false;
		
		if( (double) Current_Step_Number < middle_on_the_route)
		{
			Make_Inversion = true;
		}
		Double Rounds_Time = (double) (Steps_on_route - Current_Step_Number)/ (double) Route_Speed;
		
		
		double average = (double) Steps_on_route / (double) 2;
		average = Math.floor(average);
		
		Boolean Make_U_turn = false;
		
		int Position_Start_Step = Current_Step_Number;
		int Position_Start_Route = Current_Route_Number;
		int Position_Start_City = Current_City_Position;
...
//		int Position_End_City = City_Number;
		...
		if((double)Current_Step_Number < average)
		{
			Make_U_turn = true;
...
//		}
			Position_End_Step = Specular_Step_Number;
			Position_End_Route = Specular_Route_Number;
			Position_End_City = Current_City_Position;
			
			
		
		/**
		 * I insert the "Declare_the_Refuel" Action!
		 * 1° Action 
		 */
		ArrayList<TAction> Plans = new ArrayList<TAction>();
		ArrayList<TAction> Actions = new ArrayList<TAction>();
		for(int o=0;o<4;o++)
		{
			...
			ArrayList<Object> Params = new ArrayList<Object>();
//			Params.add(Current_City_Name.name());
...
//			Params.add(Position_Start_Step);
			Params.add(Current_City_Name.ordinal());
...
//			An_Action.Add_Post_condition(Precondition_Step);
		...
		if(false)
		{
			...
			// POSTCONDITIONS
			Postcondition_City = new TPredicate(null, BL_Current_Position_City, 
					TType_Relationship.is, Position_End_City);
			Postcondition_Route = new TPredicate(null, BL_Current_Position_Route, 
					TType_Relationship.is, Position_End_Route);
			Postcondition_Step = new TPredicate(null, BL_Current_Position_Step, 
					TType_Relationship.is, Position_End_Step);
		...
		TOption_Simulation An_Option = new TOption_Simulation(Actions, null, 0.0, quality_List);
		List<TCity> Destionations = new ArrayList<>();
		Destionations.add( Current_City_Name );
	...
	//	LocalDateTime Temp_Time = Current_Time.plusMinutes(Minutes);
		...
		results.add(An_Option);
	
		return results;
	}
	
	public ArrayList<TOption> Reasoner_Function_for_Refuel_new_3
	(		TPractical_Desire Practical_Desire, 
			HashMap<String, TBelief> Beliefs,
            HashMap<String, TRegion> Regions, 
            ArrayList<TIntention> intentions, 
...
//		Integer Start_City = Position_Start_City;
//		Integer Start_Route = Position_Start_Route;
//		Integer Start_Step = Position_Start_Step;
//		
//		Integer End_City = 0;
//		Integer End_Route = 0;
//		Integer End_Step = 0;
...
//			An_Action.Add_Post_condition(Precondition_Step);
		...
		/**
		 * The agent moves the car
		 * 2° Action: GO_TO_Step
		 */
		
		Position_End_City = Position_Start_City;
		Position_End_Route = Position_Start_Route;
		Position_End_Step = Position_Start_Step + Route_Speed;
		
		if(Position_End_Step > Steps_on_route)
		{
			Position_End_City = A_Route.Get_Destination().ordinal();
			Position_End_Route = -1;
			Position_End_Step = 0;
		}
		
		{
			...
			Postcondition_City = new TPredicate("Position_City", BL_Current_Position_City, 
					TType_Relationship.is, Position_End_City);
			Postcondition_Route = new TPredicate("Position_Route", BL_Current_Position_Route, 
					TType_Relationship.is, Position_End_Route);
			Precondition_Step = new TPredicate("Position_Step", BL_Current_Position_Step, 
					TType_Relationship.is, Position_End_Step);
			
			String Function_To_Invoke = "GO_TO_Step";
...
//			An_Action.Set_Action_Name("Initialize_Way");
		...
		Position_Start_City = Position_End_City;
		Position_Start_Route = Position_End_Route;
		Position_Start_Step = Position_End_Step;
...
//		{
			 ...
			 */
			{
				...
				Params.add( A_Destination_Station.name() );
...
//	}
	...
	public ArrayList<TOption> Reasoner_Function_for_Refuel_new_2
	(		TPractical_Desire Practical_Desire, HashMap<String, TBelief> Beliefs,
            HashMap<String, TRegion> Regions, ArrayList<TIntention> intentions, 
            IPlanner Planner)
...
//	}
	...
	public ArrayList<TOption> Reasoner_Function_for_Refuel_new_4(
//			TPractical_Desire  Practical_Desire, HashMap<String, TBelief> Beliefs,
//			TMeans_End_Reasoner_Data_Getter Means_End_Reasoner_Data_Getter)
		TPractical_Desire Practical_Desire, HashMap<String, TBelief> Beliefs, HashMap<String, TRegion> Regions,
		ArrayList<TIntention> intentions, IPlanner Planner)// ,
...
//	Common_Functions.Print("Start city for " + Practical_Desire.Get_Name() + ": " + Current_City.toString());
	...
	Integer Temp_Specular_Route_Number = 0;
	ArrayList<TPlan_Simulation> Paths = null;
	Boolean Start_New_Travel = false;
	Paths = new ArrayList<TPlan_Simulation>();
//	if(Temp_Route_Number < 0)
//	if (Temp_Step_Number == 0) 
//	{
//		Temp_Step_Number = 0;
...
//	TBelief BL_Position_Step = Beliefs.get("BL_Position_Step");
	...
	int i = 0;
//	for (TPlan_Simulation path : Paths)
//	TPlan_Simulation path = new TPlan_Simulation();
	TRoute A_Route = Map.Get_Route(Temp_Route_Number);
//	path.Routes.add(Temp_Route_Number);
//	path.Destinations.add( A_Route.Get_Departure() );
//	path.Destinations.add( A_Route.Get_Destination() );
////	EnumMap<TType_Quality_Desire, Double> Total_Weights = new EnumMap<TType_Quality_Desire, Double>(null);
...
//	TOption_Simulation An_Option = new TOption_Simulation(Actions, null, 0.0, quality_List);
	List<TCity> Destionations = new ArrayList<>();
	Destionations.add( A_Route.Get_Departure() );
	Destionations.add( A_Route.Get_Destination() );
	
	List<Integer> Numbered_Route = new ArrayList<>();
	Numbered_Route.add( Temp_Route_Number );
	Double path_Time = (double) 1;
	...
	plan.Insert_Path_by_Routes(Destionations, Numbered_Route, quality_List, path_Time);
	{
		...
		// For Any Route
		int Action_ID = 0;
		for (Integer Route_Number : plan.Routes) {
			// Now, Action stores only a Route at a time
			// An Action => A step for a Route
...
//				}
		...
		TOption_Simulation An_Option = new TOption_Simulation(Actions, null, 0.0, plan.Total_Weights);
		An_Option.Path.Copy_Plan(plan);

		// I update the time to satisfy the plan option
		Minute_of_Hours = plan.Path_Time.intValue();
		Minutes = (int) ((plan.Path_Time - Minute_of_Hours) * 60) + Minute_of_Hours * 60;
//		LocalDateTime Temp_Time = Current_Time.plusMinutes(Minutes);
		An_Option.Set_Satisfied_Time(Current_Time.plusMinutes(Minutes));
