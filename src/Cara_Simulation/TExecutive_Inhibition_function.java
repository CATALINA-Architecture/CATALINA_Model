...
public class TExecutive_Inhibition_function {
	 ...
	 */
	private boolean Fucused_Message = false; //only to show Scenario 1
...
//					Game.Press_Enter();
					...
					//this is used only to show the focusing of the agent in Scenario 1 
					if(Intention.get_Desire().get_Attentional_Goal() instanceof TFunctional_Desire)
						{
							if(Intention.get_Desire().get_Attentional_Goal().get_Name().equals("Visit_Rome"))
							{
								if(! Fucused_Message)
								{
					...
					ArrayList<TAttentional_Desire> Inhibited_Goals = this.Create_Inhibited_Goals(Intentions);
...
//								}
			...
			else
			{
				...
				TEpistemic_Desire Epistemic_Goal = (TEpistemic_Desire) Intention.get_Desire().get_Attentional_Goal();
				// If the functional belief is Belief_Destination_Station, it means that it is a Functional Goal
				// to go in a specific station, so I don't must to inhibit only specific beliefs
				Game.Print("I cannot handle TEpistemic_Goal in Create_Inhibition_Regions Function");
				
				int Index_Option = 0;
				TActive_Desire Desire = null;
				TOption Option = null;
				TAction Action = null;
...
//							int Integer_Route = (int) Action.Get_Params().getFirst();
	...
	private ArrayList<TAttentional_Desire> Create_Inhibited_Goals(ArrayList<TIntention> Intentions)
	{
		/*
		 * quali sarebbero in questi casi??
		 */
		ArrayList<TAttentional_Desire> Inhibited_Goals = new ArrayList<TAttentional_Desire>();
...
//		for (TIntention Intention: this.Agent.Get_GW().Get_Intentions())
		for (TIntention Intention: Intentions)
		{
			...
			TAttentional_Desire Attentional_Goal =  Intention.get_Desire().get_Attentional_Goal();
...
//		this.Agent.get_GW().Print_Data(1, 0);
	...
	private ArrayList<TBelief> Create_List_of_Belief__for_Destination_Station(TIntention Intention, 
			TFunctional_Desire Functional_Goal)
	{
		ArrayList<TBelief> New_All_related_Beliefs_Of_Goal = new ArrayList<TBelief>();
...
//						+ "in TShifting_Attention: "+ Belief.get_Type_Belief());
	...
	private ArrayList<TBelief> Create_List_of_Belief__for_Stimulus_Danger_on_the_Route(TEpistemic_Desire Epistemic_Goal)
	{
		ArrayList<TBelief> New_All_related_Beliefs_Of_Goal = new ArrayList<TBelief>();
//		TBelief_Base Belief = null;
		ArrayList<TBelief> All_Beliefs = this.Agent.Get_GW().Get_Beliefs_from_WMM();
...
//		Game.Print("end Create_List_of_Belief__for_Stimulus_Temporary_Closed_Route");
	...
	private ArrayList<TBelief> Create_List_of_Belief__for_Stimulus_Temporary_Closed_Route(TEpistemic_Desire Epistemic_Goal)
	{
		ArrayList<TBelief> New_All_related_Beliefs_Of_Goal = new ArrayList<TBelief>();
//		TBelief_Base Belief = null;
		ArrayList<TBelief> All_Beliefs = this.Agent.Get_GW().Get_Beliefs_from_WMM();
...
//		Game.Print("end Create_List_of_Belief__for_Stimulus_Temporary_Closed_Route");
	...
	public boolean Desire_Deletion()
	{
		...
		if(this.Updated_Desires)
		{
			...
			ArrayList<TFunctional_Desire> Functional_Goals = GW.Get_Functional_Goals();

			ArrayList<TFunctional_Desire> Inhibited_Functional_Goals = GW.Get_Inhibited_Functional_Goals();
			...
			ArrayList<TFunctional_Desire> Uninhibited_Functional_Goals = new ArrayList<TFunctional_Desire>() ;//GW.Get_UnInhibited_Goals();
			...
			ArrayList<TActive_Desire> Desires = GW.Get_Desires();
			
			// for all d in current_Desires
			Game.Print("Desires: "+Desires.size()+ "- "+Desires);
			int i=0;
//			
			ArrayList<TActive_Desire> Desire_To_Remove = new ArrayList<TActive_Desire>();
...
//					if (Desire.get_Attentional_Goal().get_Saliency() < this.Agent.Get_GW().Get_Saliency_Threshold())
					{
						Game.Print("I Delete a Desire. Desire name: "+Desire.Get_Name() + " - related Functional Goal: "+Desire.get_Attentional_Goal().get_Name());
						Game.Print("Desire Saliency is: "+Desire.get_Attentional_Goal().get_Saliency()+ " - while Saliency Threshold is: "+this.Agent.Get_E_Inhibition_function().get_Saliency_Threshold());
						Game.Print("Desire Saliency is: "+Desire.get_Attentional_Goal().get_Saliency()+ " - while Saliency Threshold is: "+this.Agent.Get_GW().Get_Saliency_Threshold());
...
//					if (Desire.get_Attentional_Goal().get_Saliency() < this.Agent.Get_GW().Get_Attention_Threshold())
					{
						Game.Print("I Delete a Desire. Desire name: "+Desire.Get_Name() + " - related Functional Goal: "+Desire.get_Attentional_Goal().get_Name());
						...
						Game.Print("Desire Saliency is: "+Desire.get_Attentional_Goal().get_Saliency()+ " - while Attention Threshold is: "+this.Agent.Get_GW().Get_Attention_Threshold());
//						GW.Delete_Desire(Desire);
						Desire_To_Remove.add(Desire);
					}
				}
