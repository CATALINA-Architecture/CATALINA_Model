...
//		Game.Print(predicates.size());
	...
	public void Set_Functional_Goals(ArrayList<TFunctional_Desire> functional_Goals)
	{
		this.Long_Memory.AddAll_Functional_Goals(functional_Goals);
		
		ArrayList<TFunctional_Desire> Functional_Goals = this.Get_Functional_Goals();
		...
		for(TFunctional_Desire Functional_Goal: Functional_Goals)
		{
			if(Functional_Goals.size() > 0)
			{
				...
				if(Functional_Goal.get_List_Green_Goal_Name() != null)
				{
				...
				if(Functional_Goal.get_List_Quality_Goal_Name() != null)
				{
	...
	public void Set_Epistemic_Goals(ArrayList<TEpistemic_Desire> epistemic_Goals)
	{
		if(epistemic_Goals.size() > 0)
		{
			this.Long_Memory.AddAll_Epistemic_Goals(epistemic_Goals);
			
			ArrayList<TEpistemic_Desire> Epistemic_Goals = this.Get_Epistemic_Goals();
			...
			for(TEpistemic_Desire Epistemic_Goal: Epistemic_Goals)
			{
				ArrayList<TBelief> List_Beliefs = new ArrayList<TBelief>();
				ArrayList<TPredicate> List_Predicates = new ArrayList<TPredicate>();
	...
	public ArrayList<TFunctional_Desire> Get_Functional_Goals()
	{
		...
		ArrayList<TAttentional_Desire> Goals = this.Long_Memory.Get_Goals();
		
		for(TAttentional_Desire Goal: Goals)
		{
	...
	{
		ArrayList<TFunctional_Desire> Inhibited_Functional_Goals = new ArrayList<TFunctional_Desire>();
		ArrayList<TAttentional_Desire> Goals = this.Long_Memory.Get_Inhibited_Goals();
...
//		Inhibited_Functional_Goals = this.Long_Memory.Get_Inhibited_Goals() 
	...
	public ArrayList<TEpistemic_Desire> Get_Epistemic_Goals()
	{
		...
		for(TAttentional_Desire Goal: Goals)
		{
			if(Goal instanceof TEpistemic_Desire)
			{
	...
	public ArrayList<TAttentional_Desire> Get_Attentional_Goals()
	{
...
//			Game.Print(Belief.get_Name()+" - "+Belief.get_Type_Belief());
	...
	public void Set_Inhibited_Goals(ArrayList<TAttentional_Desire> Inhibited_Goals)
	{
...
//		  return ste[ste.length - 1 - depth].getMethodName();
	...
	public TEpistemic_Desire Stimulus_to_Goal(TStimulus Salient_Belief)
	{
		...
		TEpistemic_Desire Epistemic_Goal = new TEpistemic_Desire(Epistemic_Goal_Name, Salient_Belief, 
...
//			this.Agent.Get_GW().Update_Goals_for_Broadcast();
		...
		TFunctional_Desire Functional_Goal =  (TFunctional_Desire)Intention.get_Desire().get_Attentional_Goal();
...
//		Functional_Goal = (TFunctional_Goal) Desire.get_Attentional_Goal();
		...
		if(Current_Station == Final_Station)
			{
			TActive_Desire Desire_to_Delete = Intention.get_Desire();
			Game.Print_Colored_Text("My intention to arrive to "+Final_Station+ " has been realized!\n"
					+"I satisfied the goal: "+Functional_Goal.get_Name(), 5);
...
//			{
				...
				Game.Gui_Map.Show_Message("Success!", "My intention to arrive to "+Final_Station+ " has been realized!\n"
						+"I satisfied the goal: "+Functional_Goal.get_Name(),
						JOptionPane.INFORMATION_MESSAGE);
//			}
			Game.Print("Now I delete the Selected Intention, with its desire and I move the Functional Goal in my goal satisfied List");
			Game.Print(Desire_to_Delete.Get_Name()+": "+Desire_to_Delete.get_Attentional_Goal().get_Saliency()+
					" - Functional Goal: "+Desire_to_Delete.get_Attentional_Goal().get_Name());
					...
					== TType_Beliefs.Belief_Come_Back_to_City)
			{
				...
				Game.Print("Intention.get_Desire(): "+Intention.get_Desire().Get_Name());
				Game.Print("TFunctional_GoalIntention.get_Desire(): "+((TFunctional_Desire)Intention.get_Desire().get_Attentional_Goal()).get_Name());
				Game.Gui_Map.Show_Message("Warning!", "I returned to the previous station: "+Current_Station,
						JOptionPane.WARNING_MESSAGE);
			}
			this.Agent.Get_GW().Delete_Intention(Intention);
			this.Agent.Get_GW().Get_Goals().remove((TAttentional_Desire)Functional_Goal);
			this.Agent.Get_GW().Get_Functional_Goals().remove(Functional_Goal);
//			this.Long_Memory.Get_Goals().re
			this.Long_Memory.Delete_Goal((TAttentional_Desire)Functional_Goal);
...
//				Game.Gui_Map.Show_Message("errore", "e' presente", JOptionPane.ERROR_MESSAGE);
			...
			this.Long_Memory.Insert_Attentional_Goals_Satisfied((TAttentional_Desire)Functional_Goal);
			...
			if(!Game.Disable_Show_Message)
			{
				Game.Gui_Map.Show_Message("Information...", "I delete the Functional goal!",
						JOptionPane.INFORMATION_MESSAGE);
			}
		}
	}
...
//			Game.Print(Bel.get_Name()+": "+Bel.get_Type_Belief()+" - "+Bel.get_Predicate().get_Subject());
		...
		TEpistemic_Desire Epistemic_Goal = null;
		for(TIntention Intent: this.Agent.Get_GW().Get_Intentions())
		{
			if(Intent.get_Desire().get_Attentional_Goal() instanceof TEpistemic_Desire)
			{
				Epistemic_Goal = (TEpistemic_Desire) Intent.get_Desire().get_Attentional_Goal();
