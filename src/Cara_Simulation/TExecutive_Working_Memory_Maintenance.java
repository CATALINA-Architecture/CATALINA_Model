...
//				Game.Print("*********************************** "+ String_TVS_Answer);
	...
	public void Manage_Correct_Movement(TPerception Perception)
	{
		...
		if(Current_Belief_Station.Predicate.get_Object_Complement() != Train_Coords.Get_City())
		{
			...
			ArrayList<TBelief> Belief_Visited_Stations = this.Agent.Get_GW().Get_Beliefs_From_Type_Belief(TType_Beliefs.Belief_Visited_Station);
...
//			}
					...
					== TType_Beliefs.Belief_Come_Back_to_City)
			{
				...
				Game.Print("TFunctional_GoalIntention.get_Desire(): "+((TFunctional_Goal)Intention.get_Desire().get_Attentional_Goal()).get_Name());
				Game.Gui_Map.Show_Message("Warning!", "I returned to the previous station: "+Current_Station,
						JOptionPane.WARNING_MESSAGE);
			}
			this.Agent.Get_GW().Delete_Intention(Intention);
			this.Agent.Get_GW().Get_Goals().remove((TAttentional_Goal)Functional_Goal);
			this.Agent.Get_GW().Get_Functional_Goals().remove(Functional_Goal);
//			this.Long_Memory.Get_Goals().re
			this.Long_Memory.Delete_Goal((TAttentional_Goal)Functional_Goal);
...
//				Game.Gui_Map.Show_Message("errore", "e' presente", JOptionPane.ERROR_MESSAGE);
			...
			this.Long_Memory.Insert_Attentional_Goals_Satisfied((TAttentional_Goal)Functional_Goal);
	...
	public TType_Route_Status Manage_Acquired_Route_Status(TPerception Perception)
	{
		...
		ArrayList<TBelief> Visited_Station = this.Agent.Get_GW().Get_Beliefs_From_Type_Belief(TType_Beliefs.Belief_Visited_Station);
...
//			Game.Print(Bel.get_Name()+": "+Bel.get_Type_Belief()+" - "+Bel.get_Predicate().get_Subject());
		...
		{
			if(Intent.get_Desire().get_Attentional_Goal() instanceof TEpistemic_Goal)
			{
				Epistemic_Goal = (TEpistemic_Goal) Intent.get_Desire().get_Attentional_Goal();
		...
		Salient_Belief = (TSalient_Belief) Beliefs_Irrelevant.getFirst();
		TPredicate Predicate = Salient_Belief.get_Predicate();
