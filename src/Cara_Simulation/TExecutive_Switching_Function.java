...
//				Game.Press_Enter();
				...
				ArrayList<TAttentional_Desire> Goals_To_Promote = new ArrayList<TAttentional_Desire>();
...
//				Game.Print("New Desires: "+Desires.size()+" - "+Desires);
				for (TFunctional_Desire goal : Uninhibited_Functional_Goals) 
				{
					boolean pass=true;
...
//							if (goal.get_Saliency() >= this.Agent.Get_GW().Get_Saliency_Threshold())
							{
								...
								Game.Print("I'm promoting the goal to Desire, inside --(Uninhibited Goals ooooooooooo )-- section (see Listing 1). Goal Name: "+goal.get_Name());
								Game.Print("I'm in Endogenous module...");
								Game.Print("Goal to promote: "+goal.get_Name());
								
								Game.Gui_Map.Show_Message("Agent Message", "The goal -- "+goal.get_Name()+" -- is in Uninhibited Goal and it has a saliency greater than Saliency Threshold: "+goal.get_Saliency()+" >= "+this.Agent.Get_E_Inhibition_function().get_Saliency_Threshold()+
										"\n I promote the Uninhibited Goal "+goal.get_Name()+" to desire", null);
...
//								Game.Get_Input("");
				...
				for (TFunctional_Desire goal : Inhibited_Functional_Goals) 
				{
...
//							if (goal.get_Saliency() >= this.Agent.Get_GW().Get_Attention_Threshold())
							{
								...
								Game.PrintLn();
								Game.Gui_Map.Show_Message("Agent Message", "The goal -- "+goal.get_Name()+" -- is in Inhibited Goal and it has a saliency greater than Attention Threshold: "+goal.get_Saliency()+" >= "+this.Agent.Get_E_Inhibition_function().get_Attention_Threshold()+
										"\n I promote the Inhibited Goal "+goal.get_Name()+" to desire", null);
//								Game.Gui_Map.Show_Message("Agent Message", "I'm promoting the goal to Desire, inside --(Inhibited goals )-- section (see Listing 1). Goal Name: "+goal.get_Name(), null);
				...
				for (TAttentional_Desire Goal: Goals_To_Promote)
				{
					Desire_Number++;
...
//				Game.Press_Enter();
				...
				//22/05/2025
				TGlobalWorkspace GW = this.Agent.Get_GW();
				TEpistemic_Desire New_Goal = GW.Stimulus_to_Goal(Stimulus);
...
//	}
	...
	private Boolean Verify_Precondition(TFunctional_Desire Goal)
	{	
...
//		}
	...
	private Boolean Check_Precondition_Visited_City_for_Belief_Destination_Station(TFunctional_Desire Goal)
	{
		Boolean result = false;
...
//		ArrayList<TBelief_Base> Beliefs = this.Agent.Get_WMM().Get_Beliefs();
	...
	private TActive_Desire Promote_to_Desire(int Desire_Number, TAttentional_Desire attentional_Goal, ArrayList<TOption> option_List )
	{
