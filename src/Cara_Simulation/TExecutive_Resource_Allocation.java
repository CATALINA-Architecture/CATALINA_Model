...
//												Game.Print("Entro qua in GO_TO_Route");
	...
	//										Integer Max_Steps = the_route.get_Route_Speed();
											...
											ArrayList<Point> Points = Game.Gui_Map.mappaPanel.Ruotes_Coords.get(Integer_Route);
											if(Integer_Start_Step<Integer_End_Step)
											{
												while(Integer_Start_Step<Integer_End_Step)
												{
													...
													Game.Gui_Map.mappaPanel.spostaMacchinaLentamente(A_Point, Time_to_Move);
												}
											}
...
//												TTriple_Object Response = new TTriple_Object();
	...
	//								TPosition_Train_Coords Post_Position2 = (TPosition_Train_Coords) Postcondition.get_Object_Complement();
									...
									Float Time_to_Move = 1.0f ;
									ArrayList<Point> Points = Game.Gui_Map.mappaPanel.Ruotes_Coords.get(Integer_Start_Route); 
									...
									if(Integer_End_Route==-1)
									{
										...
										Point A_Point = Game.Gui_Map.mappaPanel.Stations_Coords.get(Next_Station);
										Game.Gui_Map.mappaPanel.spostaMacchinaLentamente(A_Point, Time_to_Move);
									}
									break;
...
//									break;
				...
				else
				{
					...
					Game.Print_Colored_Text("Related to Desire: "+Intention.get_Desire().Get_Name(), 0);
					if(Intention.get_Desire().get_Attentional_Goal() instanceof TFunctional_Desire)
					{
						...
						Game.Print_Colored_Text("Related to Functional_Goal: "+Functional_Goal.get_Name(), 0);
					}
					else
					{
						TEpistemic_Desire Epistemic_Goal = (TEpistemic_Desire)Intention.get_Desire().get_Attentional_Goal();
						Game.Print_Colored_Text("Related to Epistemic_Goal: "+Epistemic_Goal.get_Name(), 0);
					}
				}
...
//        String String_Duration = Game.Get_Preset_Input("Closed Route Duration?:  ",number.toString(),2);
		...
		TFunctional_Desire Functional_Goal = new TFunctional_Desire("", Belief_Come_Back_to_City.get_Name(), "", 
				0.9, 40.0, 0.1, null, null, null, null, null, null, null, null);
		
		Game.Print("Functional_Goal size: "+this.Agent.Get_WMM().Get_Functional_Goals().size());
		this.Agent.Get_GW().Add_Goal(Functional_Goal);
		
		Game.Print("Functional_Goal: "+Functional_Goal.get_Name());
		Game.Print("New Functional_Goal size: "+this.Agent.Get_WMM().Get_Functional_Goals().size());
	}
