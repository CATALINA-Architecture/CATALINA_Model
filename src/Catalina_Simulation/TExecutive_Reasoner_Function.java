...
class TDesire_Compare implements Comparator<TActive_Desire> {
    @Override
    public int compare(TActive_Desire Desire0, TActive_Desire Desire1) {

    	double w0 = Desire0.Get_Attentional_Standing_Desire().Saliency;
    	double w1 = Desire1.Get_Attentional_Standing_Desire().Saliency;        
...
class TGreen_Compare implements Comparator<TOption> {
    ...
    public int compare(TOption Option0, TOption Option1) {
    	...
    	double w0 = Option0.Path.Total_Weights.get(TType_Quality_Desire.TGQ_Locomotive);
    	double w1 = Option1.Path.Total_Weights.get(TType_Quality_Desire.TGQ_Locomotive);        
        return (w0 > w1? -1 : (w0 == w1) ? 0 : 1);
    }
}
...
class TQuality_Panorama_Compare implements Comparator<TOption> {
    ...
    public int compare(TOption Option0, TOption Option1) {
    	...
    	double w0 = Option0.Path.Total_Weights.get(TType_Quality_Desire.TGQ_Panorama);
    	double w1 = Option1.Path.Total_Weights.get(TType_Quality_Desire.TGQ_Panorama);        
...
class TQuality_Velocity_Compare implements Comparator<TOption> {
    ...
    public int compare(TOption Option0, TOption Option1) {
    	...
    	double w0 = Option0.Path.Total_Weights.get(TType_Quality_Desire.TGQ_Velocity);
    	double w1 = Option1.Path.Total_Weights.get(TType_Quality_Desire.TGQ_Velocity);        

        return (w0 > w1? -1 : (w0 == w1) ? 0 : 1);
    }
}

public class TExecutive_Reasoner_Function {
	...
	public void Insert_in_List_Update_Contract()
	{
		...
		this.Agent.Get_GW().Insert_for_Update_Contract(TType_Update_Contract.Active_Desires_with_Option, this);
	}
...
//				Game.Press_Enter();
				...
				ArrayList<TActive_Desire> Desires = this.Agent.Get_GW().Get_Active_Desires();
				
				ArrayList<TActive_Desire> Desires_To_Filter = new ArrayList<TActive_Desire>();
				...
				for(TActive_Desire Desire: Desires)
				{
					...
					{
						if(Desire == Intention.Get_Active_Desire())
						{
					...
					if(Can_Analyze_Desire)
					{
						...
						TAttentional_Standing_Desire Goal = Desire.Get_Attentional_Standing_Desire();
						...
						if (Goal instanceof TFunctional_Standing_Desire) 
						{
							Game.Print("********* --------------- *************");
							Game.PrintLn();
							Game.Print(Des_Num+" - For the Desire --"+Desire.Get_Name()+"-- The Attentional Desire is a:  TFunctional_Desire");
						...
						else //if (Goal instanceof TEpistemic_Goal) 
						{
							Game.Print("********* --------------- *************");
							Game.PrintLn();
							Game.Print(Des_Num+" - For the Desire --"+Desire.Get_Name()+"-- The Attentional Desire is a:  TEpistemic_Desire");
...
//				Game.Press_Enter();
				...
				ArrayList<TActive_Desire> Desires_in_Intentions = new ArrayList<TActive_Desire>();
				ArrayList<TActive_Desire> Desires_To_Promote = new ArrayList<TActive_Desire>();
				for(TIntention Intention: Intentions)
				{
					Attentional_Goals_in_Intentions.add(Intention.Get_Active_Desire().Get_Attentional_Standing_Desire().Get_Name());
					Desires_in_Intentions.add(Intention.Get_Active_Desire());
				}
				...
				for(TActive_Desire Desire: Desires_To_Promote)
				{
					...
					{
						TFunctional_Standing_Desire Functional_Goal = (TFunctional_Standing_Desire) Desire.Get_Attentional_Standing_Desire();
					...
					{
						TEpistemic_Standing_Desire Epistemic_Goal = (TEpistemic_Standing_Desire) Desire.Get_Attentional_Standing_Desire();
					...
					Game.Print("Desire Attentional Desire Name: "+Desire.Get_Attentional_Standing_Desire().Get_Name());
...
//	    }
	...
	protected boolean Deliberate_for_Belief_Destination_Station(TActive_Desire Desire, ArrayList<TBelief> Beliefs)
	{
		boolean result = true;
		try 
		{
			...
			Game.Print("Linked to Functional Desire : "+Goal.Get_Name());
			...
			if (option_List.size()>0)
			{
				...
				if(true)
				{
					...
					Game.Print("Panorama average: "+Path.Total_Weights.get(TType_Quality_Desire.TGQ_Panorama));
					Game.Print("Locomotive average ( CO2/Route ): "+Path.Total_Weights.get(TType_Quality_Desire.TGQ_Locomotive));
					Game.Print("Speed average: "+Path.Total_Weights.get(TType_Quality_Desire.TGQ_Velocity));
...
//					if(Intention.get_Desire().get_Attentional_Goal() instanceof TFunctional_Goal)
					//this is used only to show the focusing of the agent in Scenario 2
						{
							if(Desire.Get_Attentional_Standing_Desire().Get_Name().equals("Visit_Rome"))
							{
//								if(! Fucused_Message)
								{
									String Destinations = Path.Destinations.toString();
									String Routes = Path.Routes.toString();
									String Path_Time = Path.Path_Time.toString();
									String Quality_goal_Name = Desire.Get_Attentional_Standing_Desire().Get_List_Quality_Standing_Desire().getFirst().Get_Name();
									
									
									Game.Gui_Map.Show_Message("Agent Message", "Scenario 2 - Case 2\n"+
											"\nQuality Goal Name: "+Quality_goal_Name+
											"\nDestinations: "+Destinations+
											"\nRoutes: "+Routes+
											"\nPath_Time: "+Path_Time, null);
//									Fucused_Message = true;
								}
...
//						Game.Print("Route Number: "+rotta.Route_Number + " - Route_Time: "+rotta.Get_Path_Time());
						...
						Game.Print("Departure/Destination: "+ rotta.Get_Departure()+"-"+rotta.Get_Destination()+
								...
								" - Speed: "+rotta.Get_Speed()+ " - N.pieces: "+rotta.Get_Steps_Number());
						...
						Game.Print("Speed value: "+rotta.Get_Speed().Get_Value(rotta.Get_Speed()));
						Game.Print("**********************");
					}
				}
			}
	...
	protected boolean Means_End_For_Belief_Come_Back_to_City(TActive_Desire Desire, ArrayList<TBelief> Beliefs)
	{
		///
		...
		try 
		{
			...
			Game.PrintLn();
			Game.Print("The Functional_Desire Name is: "+Functional_Goal.Get_Name());
...
//				Game.Gui_Map.Show_Message(Function_To_Invoke, "(Current_Route < 0) - Current_Route: "+Current_Route, null);
				EnumMap<TType_Quality_Desire, Double> Total_Weights = 
						new EnumMap<TType_Quality_Desire, Double>(TType_Quality_Desire.class);
				Total_Weights.put(TType_Quality_Desire.TGQ_Locomotive, 0.0);
				Total_Weights.put(TType_Quality_Desire.TGQ_Panorama, 0.0);
				Total_Weights.put(TType_Quality_Desire.TGQ_Velocity, 0.0);
...
//			Game.End_Game();
	...
	protected boolean Means_End_For_Belief_Destination_Station(TActive_Desire Desire, ArrayList<TBelief> Beliefs)
	{
...
//				i++;
	...
	private Boolean Filtering_Process(ArrayList<TActive_Desire> desires)
	{
		boolean result = true;
//		try
//		{
...
//					TAttentional_Goal Goal = Desire.get_Attentional_Goal();
					...
					Game.PrintLn();
					Game.Print("Now I apply Green and Quality Desire filters");
					Game.PrintLn();
					...
					Game.Print("N. Green Goal for this Desire: " +Functional_Goal.Get_List_Green_Standing_Desire().size());
					if(Functional_Goal.Get_List_Green_Standing_Desire().size()>0)
					{
						for(int i=0; i<Functional_Goal.Get_List_Green_Standing_Desire().size(); i++)
						{
							Game.Print("I apply the Green Desire: "+Functional_Goal.Get_List_Green_Standing_Desire().get(i).Get_Name());
							Game.Print("with its Constraint Predicate Name: "+Functional_Goal.Get_List_Green_Standing_Desire().get(i).Get_Constraint_Name());
							Filtered_Options = Green_Filter(Desire.Get_Option_List(), Functional_Goal.Get_List_Green_Standing_Desire().get(i));
					...
					Game.Print("N. Quality Desire for this desire: "+Functional_Goal.Get_List_Quality_Standing_Desire().size());
					if(Functional_Goal.Get_List_Quality_Standing_Desire().size()>0)
					{
						for(int i=0; i<Functional_Goal.Get_List_Quality_Standing_Desire().size(); i++)
						{
							Game.Print("I apply the Quality Desire: "+Functional_Goal.Get_List_Quality_Standing_Desire().get(i).Get_Name());
							Game.Print("with its Constraint Predicate Name: "+Functional_Goal.Get_List_Quality_Standing_Desire().get(i).Get_Constraint_Name());
							Filtered_Options = Quality_Filter(Desire.Get_Option_List(), Functional_Goal.Get_List_Quality_Standing_Desire().get(i));
							Desire.Set_Option_List(Filtered_Options);
						}
					...
					Game.Print("Functional Desire Name for the Desire is : "+Desire.Get_Attentional_Standing_Desire().Get_Name());
...
//	private ArrayList<Plan> Green_Filter(ArrayList<Plan> Paths, TGreen_Goal Green_Goal)
	private ArrayList<TOption> Green_Filter(ArrayList<TOption> Options, TGreen_Standing_Desire Green_Goal)
	{
...
//		Game.Print(Green_Goal.get_Constraint().get_Object_Complement());
		...
		switch (Green_Goal.Get_Constraint().Get_Relationship())
		{
			case TType_Relationship.minor_equal:
				...
				Options.removeIf( n -> n.Path.Total_Weights.get(TType_Quality_Desire.TGQ_Locomotive) > Double_Value_Green_Goal );
...
//				New_Options.addAll(Options);
				break;
			default:
				Game.Print("I cannot handle Green_Desire Complement_Object: "+Green_Goal.Get_Constraint().Get_Object_Complement());
		}
...
//	private ArrayList<Plan> Quality_Filter(ArrayList<Plan> Paths, TQuality_Goal Quality_Goal)
	private ArrayList<TOption> Quality_Filter(ArrayList<TOption> Options, TQuality_Standing_Desire Quality_Goal)
	{
...
//		Game.Print("--- Quality_Goal.get_Constraint(): "+Quality_Goal.get_Constraint());
		...
		Game.Print("--- Value_Quality_Desire: "+Value_Quality_Goal);
		switch(Quality_Goal.Get_Type_Quality_Goal())
		{
		case TType_Quality_Desire.TGQ_Panorama:
...
//				int i=0;
				...
				break;			
			default:
				Game.Print("I cannot handle Quality_Desire Complement_Objecy: "+Quality_Goal.Get_Constraint().Get_Object_Complement());
			}
			
			break;
		case TType_Quality_Desire.TGQ_Velocity:
			switch (Value_Quality_Goal)
			{
			case "moderate":
				Collections.sort(Options, new TQuality_Panorama_Compare());
...
//				if(Options.size()>0)
//				{
//					New_Options.add(Options.get((int) Math.ceil((Options.size()/2))));
//				}
				if(Options.size()<3)
				{
					New_Options.addAll(Options);
				...
				{
					//I have to get the middle sublist
					int size = Options.size();
					
					int Size_Sub_list = size / 3;
			        int rest = size % 3;
			        int startIndex = 0;
			        
			        // Create the first part
//			        List<ArrayList<String>> parte1 = Options.subList(startIndex, startIndex + dimensioneParte);
//			        New_Options.addAll(Options.subList(startIndex, startIndex + Size_Sub_list));
			        startIndex += Size_Sub_list;

			        // Create the second part
			        New_Options.addAll(Options.subList(startIndex, startIndex + Size_Sub_list));
			        startIndex += Size_Sub_list;

//			        // Creates the third part (also manages any remainder)
//			        New_Options.addAll(Options.subList(startIndex, size));
//			        startIndex += Size_Sub_list;
			...
			break;
		default:
			Game.Print("I cannot handle Quality_Desire Type: "+Quality_Goal.Get_Type_Quality_Goal());
			Game.End_Game();
	...
	public Boolean Deliberate_for_Stimulus_Temporary_Closed_Route(TActive_Desire Desire)
	{
		...
		try 
		{
			...
			TEpistemic_Standing_Desire Epistemic_Goal = (TEpistemic_Standing_Desire) Desire.Get_Attentional_Standing_Desire(); 
			TStimulus Salient_Belief = (TStimulus) Epistemic_Goal.Get_Belief();
	...
	protected boolean Means_End_For_Stimulus_Danger_on_the_Route(TActive_Desire Desire, ArrayList<TBelief> Beliefs)
	{
		boolean result = true;
		try 
		{
	...
	protected boolean Means_End_For_Stimulus_Status_Route(TActive_Desire Desire, ArrayList<TBelief> Beliefs)
	{
		boolean result = true;
		try 
		{
